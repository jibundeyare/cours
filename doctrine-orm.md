...
Une entité `Foo` ne peut être rattachée qu'à une seule entité `Bar`.
Et une entité `Bar` ne peut être rattachée qu'à une seule entité `Foo`.

Exemple : une voiture ne peut être rattachée qu'à une seule plaque d'immatriculation.
Et une plaque d'immatriculation ne peut être rattachée qu'à une seule voiture.
...
class Foo
{
    ...
    /**
     * @ORM\OneToOne(targetEntity="App\Entity\Bar", inversedBy="foo")
     */
    private $bar;
...
class Bar
{
    ...
    /**
     * @ORM\OneToOne(targetEntity="App\Entity\Foo", mappedBy="bar")
     */
    private $foo;
...
Une entité `Foo` peut être rattachée à plusieurs entités `Bar`.
Mais une entité `Bar` ne peut être rattachée qu'à une seule entité `Foo`.

Exemple : une entreprise peut avoir plusieurs voitures de fonction.
Mais une voiture de fonction ne peut être rattachée qu'à une seule entreprise.
...
Une entité `Foo` ne peut être rattachée qu'à une seule entité `Bar`.
Mais une entité `Bar` peut être rattachée à plusieurs entités `Foo`.
...
Une entité `Foo` peut être rattachée à plusieurs entités `Baz`.
Et une entité `Baz` peut être rattachée à plusieurs entités `Foo`.
