...
Note : il existe une version obsolète de ce cours qui utilise une version de Symfony 3.4 qui a une structure de dossier compatible avec Symfony 2.x et incompatible avec Symfony 4.x :

- [symfony-3.4-deprecated.md](symfony-3.4-deprecated.md)

## Prérequis

Avoir de pouvoir installer Symfony, vous avoir installé la commande `symfony` au préalable.
Pour la procédure, voir [symfony-installer.md](symfony-installer.md).
...
Voici la ligne pour le user `root`, sans mot de passe, une connection sur le port `3306` (MariaDB) sur le même serveur, la BDD `my_project` :

    DATABASE_URL=mysql://root:@127.0.0.1:3306/my_project

#### MAMP

Voici la ligne pour le user `root`, le mot de passe `root`, une connection sur le port `8889` (MariaDB) sur le même serveur, la BDD `my_project` :

    DATABASE_URL=mysql://root:root@127.0.0.1:8889/my_project

#### Autre

Voici la ligne pour le user `root`, le mot de passe `123`, une connection sur le port `3306` (MariaDB) sur le même serveur, la BDD `my_project` :
...
Relancer le serveur web avec la commande `symfony serve` si nécessaire.
...
Pour en savoir plus, voir [doctrine-orm.md](doctrine-orm.md).
...
Pour savoir quelle version de MariaDB ou MySQL vous avez, tapez la commande suivante :
...
Vous verrez que si vous variez le paramètre de la route `main_hello`, le changement sera impacté dans la page :

- [http://localhost:8000/hello/Titi](http://localhost:8000/hello/Titi)

### Génération d'URL

Pour générer une URL dans un template, voir la section « Génération d'URL » dans [twig.md](twig.md).

Pour générer une URL dans un contrôleur vous pouvez utiliser la fonction `generateUrl()`.

Exemples :

    // générer l'URL de la route main_index (la page d'accueil)
    $indexUrl = $this->generateUrl('main_index');

    // générer l'URL de la route main_hello avec des paramètres
    $helloUrl = $this->generateUrl('main_hello', ['name' => 'Toto']);

### Redirection

Il est possible de rediriger un navigateur vers une autre route.

Vous avez deux options :

- rediriger vers une route de la même application
- rediriger vers une URL de la même application ou d'un autre site

Pour rediriger sur une autre route :

        /**
         * @Route("/foo", name="main_foo")
         */
        public function foo(Request $request)
        {
            // ...

            // redirection vers la route main_index
            return $this->redirectToRoute('main_index');
        }

        /**
         * @Route("/bar", name="main_bar")
         */
        public function bar(Request $request)
        {
            // ...

            // redirection vers la route main_hello avec des paramètres
            return $this->redirectToRoute('main_hello', ['name' => 'Toto']);
        }

Pour plus d'exemples, voir [Redirecting](https://symfony.com/doc/current/controller.html#redirecting)
...
## Upgrade to Symfony 3.4 with Flex
